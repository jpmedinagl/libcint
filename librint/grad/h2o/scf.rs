...
fn sort(n: usize, eig: &mut [f64], U: &mut [f64]) {
    for i in (0..n).rev() {
        for j in 0..i {
            if (eig[j] > eig[j + 1]) {
                let tmp = eig[j];
...
fn find_X(n: usize, S: &mut [f64], X: &mut [f64], Xdag: &mut [f64]) {
...
fn calc_F(n: usize, P: &mut [f64], two: &mut [f64], H: &mut [f64], G: &mut [f64], F: &mut [f64]) {
    G.fill(0.0);
    F.fill(0.0);
...
fn calc_Fprime(n: usize, F: &mut [f64], X: &mut [f64], Xdag: &mut [f64], mut Fprime: &mut [f64]) {
    let mut inter = vec![0.0; n * n];
    matmult(n, Xdag, F, &mut inter);
    matmult(n, &mut inter, X, &mut Fprime);
}

fn diag_F(n: usize, Fprime: &mut [f64], X: &mut [f64], C: &mut [f64], epsilon: &mut [f64]) {
    // diagonalize Fprime
...
fn RHF(
    ...
    int_cart(
        natm, nbas, nshells, atm, bas, env, &mut S, &mut T, &mut V, &mut H, &mut two,
    );
...
fn main() {
    ...
    let P = RHF(
        natm, nbas, nelec, nshells, &mut atm, &mut bas, &mut env, imax, conv,
    );
